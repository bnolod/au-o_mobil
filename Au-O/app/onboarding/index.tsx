    ...
    <OnboardingProvider>
      <View className="h-screen-safe overflow-y-hidden w-full mx-auto">
        ...
        <View className=" flex flex-col justify-between pb-5">
          <View className="basis-8/12 flex flex-col-reverse items-center justify-center">
            <Carousel
              ...
              data={[
                {
                  ...
                  image: (
                    <View style={Platform.OS === 'ios' ? { transform: [{ scale: 1.2 }] } : {}}>
                      <SvgSlide1Image theme={colorScheme} />
                    </View>
                  ),
                },
                {
              ...
              renderItem={({ item }) => {
                return <Slide text={item.text} image={item.image} />;
              }}
            />
          </View>
        </View>
        <CallToAction
          ...
          language={language}
          index={index}
          onPress={index !== 2 ? () => carouselRef.current?.next() : () => router.replace('/(auth)/register')}
        />
      </View>
    </OnboardingProvider>
  );
